mpss-micmgmt-python: MicMgmt Python Bindings
===================================

    micmgmt is a Python package that provides bindings for the MicMgmt SDK library.
    This package is exported for our external customers or cluster ISVs to aid in
    ...
    Using the micmgmt package allows operations such as monitoring voltage and
...
---------------
    ...
    Internally, the micmgmt bindings load libmicmgmt.so, the shared library for
    for the MicMgmt SDK, which is in turn dependent on libscif.so. Both of these libraries
    are included in our RPM packages. Please make sure you have both of them installed.

    The micmgmt package ships only two classes:
        -MicDevice: This class will channel all communications to the specified
...
---------------
   ...
   Use your package manager to install the mpss-micmgmt-python RPM.

2. Tutorial
-----------

    Writing code using the micmgmt package is very simple. The first step is creating an
    instance of MicDevice. The constructor for this class expects an integer specifying the
...
-----------------------------------------------------------------------------------------------
    ...
    documentation by calling 'help(micmgmt)' after having imported the package. The
    documentation for each update method includes a list of refreshable values.

3. More information
--------------------

    For further information, such as a full list of available methods and
...
-----------------------------------------------------------------------------------------------
        - /usr/share/doc/micmgmt/examples/examples.py
        - /usr/share/doc/micmgmt/examples/example_version.py
...
------------
    ...
    The following example code is taken from /usr/share/doc/micmgmt/examples/examples.py
