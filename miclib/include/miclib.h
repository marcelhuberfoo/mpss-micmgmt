...
/* if in linux or static library in windows */
#ifdef WIN32
...
typedef enum _error_code {
    ...
    E_MIC_RAS_ERROR,
    E_MIC_STACK,
    E_MIC_GET_WMI_PARAM_VALUE_FAILED,
    E_NO_WINDOWS_SUPPORT,
    E_WMI_FAILED
} mic_error_code;
...
int mic_get_device_at_index(struct mic_devices_list *devices, int
                            index, int *device);
int mic_open_device(struct mic_device **device, uint32_t
                    device_num);
...
int mic_get_sysfs_attribute(struct mic_device *mdh, const
                            char *entry, char *value, size_t *size);
int mic_is_ras_avail(struct mic_device *mdh, int *ras_avail);

/* Flash operations requiring maint mode */
int mic_flash_update_start(struct mic_device *mdh, void *buf, size_t
                           bufsize, struct mic_flash_op **desc);
int mic_flash_update_done(struct mic_flash_op *desc);
int mic_flash_read_start(struct mic_device *mdh, void *buf, size_t
                         bufsize, struct mic_flash_op **desc);
int mic_flash_read_done(struct mic_flash_op *desc);
int mic_set_ecc_mode_start(struct mic_device *mdh, uint16_t
                           ecc_enabled, struct mic_flash_op **desc);
int mic_set_ecc_mode_done(struct mic_flash_op *desc);
int mic_get_flash_status_info(struct mic_flash_op *desc, struct
...
int mic_free_flash_status_info(struct
                               mic_flash_status_info *status);
int mic_get_flash_vendor_device(struct mic_device *mdh, char *buf,
                                size_t *size);

/* Flash operations not requiring maint mode */
int mic_flash_size(struct mic_device *mdh, size_t *size);
int mic_flash_active_offs(struct mic_device *mdh, off_t *active);
int mic_flash_version(struct mic_device *mdh, void *buf, char *str, size_t
                      size);

/* Pci configuration */
int mic_get_pci_config(struct mic_device *mdh, struct
                       mic_pci_config **conf);
...
int mic_get_pci_class_code(struct mic_pci_config *conf, char *class_code,
                           size_t *size);
int mic_free_pci_config(struct mic_pci_config *conf);

/* Thermal info */
int mic_get_thermal_info(struct mic_device *mdh, struct
                         mic_thermal_info **thermal);
...
int mic_get_smc_fwversion(struct mic_thermal_info *thermal, char *ver,
                          size_t *size);
int mic_is_smc_boot_loader_ver_supported(struct
                                         mic_thermal_info *thermal,
                                         int *supported);
...
/* Device memory info */
int mic_get_memory_info(struct mic_device *mdh, struct
                        mic_device_mem **meminfo);
...
/* Processor info */
int mic_get_processor_info(struct mic_device *mdh, struct
                           mic_processor_info **processor);
...
int mic_get_processor_type(struct mic_processor_info *processor,
                           uint16_t *type);
int mic_get_processor_steppingid(struct
                                 mic_processor_info *processor, uint32_t *id);
...
/* Uos core info */
int mic_get_cores_info(struct mic_device *mdh, struct
                       mic_cores_info **cores);
...
/* Version info*/
int mic_get_version_info(struct mic_device *mdh, struct
...
/* Power limits */
int mic_get_power_limit(struct mic_device *mdh, struct
                        mic_power_limit **limit);
...
/* Memory utilization */
int mic_get_memory_utilization_info(struct mic_device *mdh, struct
                                    mic_memory_util_info **memory);
int mic_get_total_memory_size(struct mic_memory_util_info *memory,
                              uint32_t *total_size);
int mic_get_available_memory_size(struct
                                  mic_memory_util_info *memory,
                                  uint32_t *avail_size);
int mic_get_memory_buffers_size(struct mic_memory_util_info *memory,
                                uint32_t *bufs);
int mic_free_memory_utilization_info(struct
                                     mic_memory_util_info *memory);

/* Core utilization */
int mic_alloc_core_util(struct mic_core_util **cutil);
int mic_update_core_util(struct mic_device *mdh, struct
                         mic_core_util *cutil);
...
/* Turbo info */
int mic_get_turbo_state_info(struct mic_device *mdh, struct
                             mic_turbo_info **turbo);
...
/* Throttle state info */
int mic_get_throttle_state_info(struct mic_device *mdh, struct
                                mic_throttle_state_info **ttl_state);
int mic_get_thermal_ttl_active(struct
                               mic_throttle_state_info *ttl_state, int *active);
int mic_get_thermal_ttl_current_len(struct
                                    mic_throttle_state_info *
                                    ttl_state, uint32_t *current);
int mic_get_thermal_ttl_count(struct
                              mic_throttle_state_info *ttl_state,
                              uint32_t *count);
int mic_get_thermal_ttl_time(struct
                             mic_throttle_state_info *ttl_state,
                             uint32_t *time);
int mic_get_power_ttl_active(struct
                             mic_throttle_state_info *ttl_state, int *active);
int mic_get_power_ttl_current_len(struct
                                  mic_throttle_state_info *ttl_state,
                                  uint32_t *current);
int mic_get_power_ttl_count(struct
                            mic_throttle_state_info *ttl_state,
                            uint32_t *count);
int mic_get_power_ttl_time(struct
                           mic_throttle_state_info *ttl_state, uint32_t *time);
int mic_free_throttle_state_info(struct
                                 mic_throttle_state_info *ttl_state);

/* Uos power management config */
int mic_get_uos_pm_config(struct mic_device *mdh, struct
                          mic_uos_pm_config **pm_config);
